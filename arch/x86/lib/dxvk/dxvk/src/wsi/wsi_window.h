...
/* OsitoK W5.2: replaces upstream wsi/wsi_window.h's <windows.h> include
 * with a minimal RECT typedef. HWND/HMONITOR/LONG come from
 * osito_compat/dxvk_compat.h (force-included). */
...
namespace dxvk::wsi {
  ...
  /**
    ...
    *
    * OsitoK: keep upstream layout (the win.* fields) so unconditional
    * accesses in dxvk_swapchain_blitter / dxvk_presenter typecheck. */
