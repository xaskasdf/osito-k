...
static inline void okvk_(const char* s){ unsigned long n=0; while(s[n])++n; write(2,s,n); }
#define CHECK_FEATURE_NEED(feature) \
  ( (m_deviceFeatures.feature || !required.feature) \
       ? true : (okvk_("[okvk] MISSING feature: " #feature "\n"), false) )
