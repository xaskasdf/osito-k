...
zink_get_physical_device_info(struct zink_screen *screen) 
{
   ...
   bool support_KHR_maintenance8 = false;
   ...
   if (result != VK_SUCCESS) {
      ...
      if (num_extensions > 0) {
         ...
         for (uint32_t i = 0; i < num_extensions; ++i) {
            ...
            if (!strcmp(extensions[i].extensionName, "VK_KHR_maintenance8")) {
               support_KHR_maintenance8 = true;
            }
   ...
   if (support_KHR_maintenance8) {
         info->maint8_feats.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_8_FEATURES_KHR;
         info->maint8_feats.pNext = info->feats.pNext;
         info->feats.pNext = &info->maint8_feats;
      }
   ...
   info->have_KHR_maintenance7 |= support_KHR_maintenance7
         ;
   info->have_KHR_maintenance8 |= support_KHR_maintenance8
         ;
...
&& (info->shader_derivs_feats.computeDerivativeGroupLinear);
   ...
   if (info->have_KHR_maintenance8) {
       info->extensions[num_extensions++] = "VK_KHR_maintenance8";
   }
...
zink_verify_device_extensions(struct zink_screen *screen)
{
   ...
   if (screen->info.have_KHR_maintenance8) {
   }
...
#endif
#ifdef _WIN32
      if (!screen->vk.CmdSetDepthClampRangeEXT) {
#ifndef NDEBUG
         screen->vk.CmdSetDepthClampRangeEXT = (PFN_vkCmdSetDepthClampRangeEXT)zink_stub_CmdSetDepthClampRangeEXT;
#else
         screen->vk.CmdSetDepthClampRangeEXT = (PFN_vkCmdSetDepthClampRangeEXT)zink_stub_function_not_loaded;
#endif
      }
#endif
   }
...
void VKAPI_PTR
zink_stub_CmdSetDepthClampRangeEXT()
{
   mesa_loge("ZINK: vkCmdSetDepthClampRangeEXT is not loaded properly!");
   abort();
}
