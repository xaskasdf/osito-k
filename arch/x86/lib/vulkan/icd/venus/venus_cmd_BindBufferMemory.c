...
#define VN_CMD_TYPE_vkBindBufferMemory 28u
...
int venus_cmd_encode_BindBufferMemory(
    ...
    vcw_wr_i32(&wr, (int32_t)VN_CMD_TYPE_vkBindBufferMemory);
    ...
    if (log_count < 16u) {
        log_count++;
        printf("[VBB] reply cmd=%u vk=%u buf=%llu mem=%llu off=%llu\n",
               reply_cmd, vk_result, (unsigned long long)buf_id,
               (unsigned long long)mem_id, (unsigned long long)memory_offset);
    }
    if (reply_cmd != VN_CMD_TYPE_vkBindBufferMemory)
        return -5;
