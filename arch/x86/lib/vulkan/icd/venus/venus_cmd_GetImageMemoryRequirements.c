...
#define VN_CMD_TYPE_vkGetImageMemoryRequirements 31u
...
int venus_cmd_encode_GetImageMemoryRequirements(
    ...
    vcw_wr_i32(&wr, (int32_t)VN_CMD_TYPE_vkGetImageMemoryRequirements);
    ...
    if (reply_cmd != VN_CMD_TYPE_vkGetImageMemoryRequirements || !present)
        return -5;
    ...
    if (log_count < 16u) {
        log_count++;
        printf("[VGIMR] img=%llu size=%llu align=%llu bits=0x%x bytes=%u\n",
