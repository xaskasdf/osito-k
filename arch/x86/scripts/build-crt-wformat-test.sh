...
out_dir=${1:-"$x86_dir/build/test-crt-wformat"}
for arch in 32 64; do
    ...
    "${CLANG:-clang}" --target="$target" -O2 -msse2 -ffreestanding \
        ...
        "$x86_dir/test/crt_wformat_pe.c" -o "$dir/wformat.obj"
    "${LLD_LINK:-lld-link}" /machine:"$machine" /subsystem:console \
        /entry:mainCRTStartup /nodefaultlib /fixed /nxcompat /opt:ref \
        /out:"$dir/crt_wformat_pe$arch.exe" "$dir/wformat.obj" "$dir/kernel32.lib"
    printf '%s\n' "$dir/crt_wformat_pe$arch.exe"
done
