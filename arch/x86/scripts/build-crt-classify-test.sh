...
out_dir=${1:-"$x86_dir/build/test-crt-classify"}
for arch in 32 64; do
    dir="$out_dir/pe$arch"
    mkdir -p "$dir"
    if [ "$arch" = 32 ]; then
        target=i686-pc-windows-msvc; machine=x86; dll_machine=i386
        cp "$x86_dir/test/crt-fp-kernel32.def" "$dir/kernel32.def"
    else
        target=x86_64-pc-windows-msvc; machine=x64; dll_machine=i386:x86-64
        sed 's/@[0-9][0-9]*$//' "$x86_dir/test/crt-fp-kernel32.def" > "$dir/kernel32.def"
    fi
    ...
    "${CLANG:-clang}" --target="$target" -O2 -msse2 -ffreestanding \
        ...
        "$x86_dir/test/crt_classify_pe.c" -o "$dir/classify.obj"
    "${LLD_LINK:-lld-link}" /machine:"$machine" /subsystem:console \
        /entry:mainCRTStartup /nodefaultlib /fixed /nxcompat /opt:ref \
        /out:"$dir/crt_classify_pe$arch.exe" "$dir/classify.obj" "$dir/kernel32.lib"
    printf '%s\n' "$dir/crt_classify_pe$arch.exe"
done
