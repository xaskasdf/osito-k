...
"$clang_bin" --target=i686-pc-windows-msvc -O2 -ffreestanding \
    ...
    "$x86_dir/test/dsound3d_pe32.c" \
    -o "$out_dir/dsound3d_pe32.obj"

"$link_bin" /machine:x86 /subsystem:console /entry:mainCRTStartup \
    /nodefaultlib /fixed /opt:ref \
    /out:"$out_dir/dsound3d_pe32.exe" \
    "$out_dir/dsound3d_pe32.obj" "$out_dir/kernel32.lib" \
    "$out_dir/dsound.lib"

printf '%s\n' "$out_dir/dsound3d_pe32.exe"
