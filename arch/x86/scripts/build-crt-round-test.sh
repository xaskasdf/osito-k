...
out_dir=${1:-"$x86_dir/build/test-crt-round"}
for arch in 32 64; do
    ...
    "${CLANG:-clang}" --target="$target" -O2 -msse2 -ffreestanding \
        ...
        "$x86_dir/test/crt_round_pe.c" -o "$dir/round.obj"
    "${LLD_LINK:-lld-link}" /machine:"$machine" /subsystem:console \
        /entry:mainCRTStartup /nodefaultlib /fixed /nxcompat /opt:ref \
        /out:"$dir/crt_round_pe$arch.exe" "$dir/round.obj" "$dir/kernel32.lib"
    printf '%s\n' "$dir/crt_round_pe$arch.exe"
done
