...
# NET=vmnet-shared (or NET=vmnet) switches the netdev backend from
# QEMU's user-mode slirp to Apple's vmnet framework.  vmnet-shared
# gives the guest a real NAT'd interface on a host bridge — no slirp
# packet drops or NAT exhaustion.  Requires sudo to attach.
#
# Usage:
#   sudo NET=vmnet bash scripts/qemu-agent-hvf.sh
if [ "${NET:-}" = "vmnet" ] || [ "${NET:-}" = "vmnet-shared" ]; then
    if [ "$(id -u)" -ne 0 ]; then
        echo "ERROR: NET=vmnet needs sudo (vmnet framework requires root)" >&2
        exit 1
    fi
...
qemu-system-x86_64 -accel hvf -cpu host \
    ...
    ${NETDEV_ARGS:--netdev user,id=net0,hostfwd=udp::7778-:7777,hostfwd=tcp::50052-:50052} \
    -object filter-dump,id=f0,netdev=net0,file=/tmp/qemu-net.pcap \
