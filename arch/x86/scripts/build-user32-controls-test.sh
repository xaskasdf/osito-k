...
out_dir=${1:-"$x86_dir/build/test-user32-controls"}
for arch in 32 64; do
    ...
    for dll in kernel32 user32 gdi32; do
        source="$x86_dir/test/user32-controls-$dll.def"
        if [ "$arch" = 64 ]; then
