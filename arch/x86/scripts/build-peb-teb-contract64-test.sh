...
"$clang_bin" --target=x86_64-pc-windows-msvc -O2 -ffreestanding \
    ...
    "$x86_dir/test/peb_teb_contract_pe64.c" \
    -o "$out_dir/peb_teb_contract_pe64.obj"
"$clang_bin" --target=x86_64-pc-windows-msvc -O2 -ffreestanding \
    -fno-builtin -fno-stack-protector -fno-ident -c \
    "$x86_dir/test/peb_teb_contract_launcher_pe64.c" \
    -o "$out_dir/peb_teb_contract_launcher_pe64.obj"

"$link_bin" /machine:x64 /subsystem:console,6.1 /entry:mainCRTStartup \
    /nodefaultlib /fixed /opt:ref \
    /out:"$out_dir/peb_teb_contract_pe64.exe" \
    "$out_dir/peb_teb_contract_pe64.obj" \
    "$out_dir/kernel32.lib" "$out_dir/ntdll.lib"
"$link_bin" /machine:x64 /subsystem:console,6.1 /entry:mainCRTStartup \
    /nodefaultlib /fixed /opt:ref \
    /out:"$out_dir/peb_teb_contract_launcher_pe64.exe" \
    "$out_dir/peb_teb_contract_launcher_pe64.obj" \
    "$out_dir/kernel32.lib"

printf '%s\n' "$out_dir/peb_teb_contract_pe64.exe"
printf '%s\n' "$out_dir/peb_teb_contract_launcher_pe64.exe"
