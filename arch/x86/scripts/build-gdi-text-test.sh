...
out_dir=${1:-"$x86_dir/build/test-gdi-text"}
for arch in 32 64; do
    ...
    for dll in kernel32 user32 gdi32; do
        source="$x86_dir/test/gdi-text-$dll.def"
        [ "$dll" != kernel32 ] || source="$x86_dir/test/gdi-paint-kernel32.def"
        if [ "$arch" = 64 ]; then
