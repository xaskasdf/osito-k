...
"$clang_bin" --target=i686-pc-windows-msvc -O2 -ffreestanding \
    ...
    "$x86_dir/test/peb_teb_contract_pe32.c" \
    -o "$out_dir/peb_teb_contract_pe32.obj"

"$link_bin" /machine:x86 /subsystem:console /entry:mainCRTStartup \
    /nodefaultlib /fixed /opt:ref \
    /out:"$out_dir/peb_teb_contract_pe32.exe" \
    "$out_dir/peb_teb_contract_pe32.obj" \
    "$out_dir/kernel32.lib" "$out_dir/ntdll.lib"

printf '%s\n' "$out_dir/peb_teb_contract_pe32.exe"
