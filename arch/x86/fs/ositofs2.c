...
static uint32_t file_open_refs[OSFS2_MAX_FILES_LARGE];
static uint64_t file_revisions[OSFS2_MAX_FILES_LARGE];
...
static bool osfs2_journal_record_valid(osfs2_journal_record_t *record)
{
    ...
    for (uint32_t i = 0; i < record->entry_count; i++) {
        if (record->slots[i] >= osfs2_format_max_files(
                record->after_super.version)) return false;
...
int osfs2_mount(uint64_t part_offset, uint64_t part_bytes)
{
    ...
    data_start = osfs2_format_data_start_blk(superblock.version, blk_size);
    file_capacity = osfs2_format_max_files(superblock.version);
    file_table_size = osfs2_format_filetab_size(superblock.version);
    crc_table_offset = osfs2_format_crctab_off(superblock.version);
    ...
    crc_table = NULL;
    if (osfs2_format_has_crc_table(superblock.version)) {
...
#endif
    if (!mounted || !osfs2_format_has_layer_index(superblock.version) ||
