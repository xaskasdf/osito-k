...
#define SYS_DEBUG_WATCH     522  /* GTA buffer corruption diagnostics */
...
static void vma_free_pages_in_cr3(vma_t *v, uint64_t cr3, uint32_t pid)
{
    if (v->type == VMA_SHARED_STACK ||
        ...
        serial_puts(" phys=0x");
        serial_puthex(VIRT_TO_PHYS(v->base), 16);
        serial_puts(" pages=");
        serial_putdec(v->pages);
        serial_puts(" type=");
        serial_putdec(v->type);
        serial_puts(" rip=0x");
...
static int64_t sys_open(uint64_t path_addr, uint64_t flags, uint64_t mode)
{
    ...
    if (f->node.fs_version == 2 && osfs2_file_retain(f->node.data) < 0) {
        memset(f, 0, sizeof(*f));
        return -EIO;
    }
                         ...
                         uint64_t flags, uint64_t fd, uint64_t offset)
{
    ...
    /* Syscalls execute on the caller's stack. A scheduler CR3 switch must
     * therefore keep pthread stacks mapped while the ISR unwinds. */
    if (flags & MAP_STACK) {
        ...
        vma_table[vi].owner = proc_current();
        serial_puts("[VMA-STACK] alloc owner=");
        serial_putdec((uint64_t)proc_current_pid());
        serial_puts(" base=0x");
        serial_puthex(result, 16);
        serial_puts(" phys=0x");
        serial_puthex((uint64_t)phys, 16);
        serial_puts(" pages=");
        serial_putdec(npages);
        serial_puts("\n");
        return (int64_t)result;
    }
