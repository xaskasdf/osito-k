/*
 * OsitoK AArch64 -- Intel HDA (High Definition Audio) Driver
 *
 * Ported from x86-64 version for QEMU virt machine.
 * Uses MMIO via uintptr_t, dsb() barriers, yield spin delay.
 */
...
static void hda_detect_format(void)
{
    ...
    if (pcm & HDA_RATE_192KHZ) {
        hda.sample_rate = 192000; fmt_rate = HDA_FMT_BASE_48 | HDA_FMT_MUL_4;
    } else if (pcm & HDA_RATE_96KHZ) {
        hda.sample_rate = 96000; fmt_rate = HDA_FMT_BASE_48 | HDA_FMT_MUL_2;
    } else if (pcm & HDA_RATE_1764KHZ) {
        hda.sample_rate = 176400; fmt_rate = HDA_FMT_BASE_441 | HDA_FMT_MUL_4;
    } else if (pcm & HDA_RATE_882KHZ) {
        hda.sample_rate = 88200; fmt_rate = HDA_FMT_BASE_441 | HDA_FMT_MUL_2;
    } else if (pcm & HDA_RATE_48KHZ) {
        hda.sample_rate = 48000; fmt_rate = HDA_FMT_BASE_48 | HDA_FMT_MUL_1;
    } else if (pcm & HDA_RATE_441KHZ) {
        hda.sample_rate = 44100; fmt_rate = HDA_FMT_BASE_441 | HDA_FMT_MUL_1;
    } else {
        hda.sample_rate = 48000; fmt_rate = HDA_FMT_BASE_48 | HDA_FMT_MUL_1;
    }

    uint16_t fmt_bits;
    if (pcm & HDA_BITS_32) { hda.bits_per_sample = 32; fmt_bits = HDA_FMT_32BIT; }
    else if (pcm & HDA_BITS_24) { hda.bits_per_sample = 24; fmt_bits = HDA_FMT_24BIT; }
    else if (pcm & HDA_BITS_20) { hda.bits_per_sample = 20; fmt_bits = HDA_FMT_20BIT; }
    else if (pcm & HDA_BITS_16) { hda.bits_per_sample = 16; fmt_bits = HDA_FMT_16BIT; }
    else { hda.bits_per_sample = 16; fmt_bits = HDA_FMT_16BIT; }
...
static int hda_stream_init(void)
{
    ...
    /* Allocate audio buffer (page-aligned via mem_alloc_pages) */
    uint32_t bytes_per_sample = (hda.bits_per_sample + 7) / 8;
    hda.buf_size = hda.sample_rate * bytes_per_sample * 2;  /* stereo, 1 second */
    uint64_t buf_pages = (hda.buf_size + 4095) / 4096;
    hda.audio_buf = (int16_t *)mem_alloc_pages(buf_pages);
    if (!hda.audio_buf) {
    ...
    /* Allocate BDL (page-aligned, exceeds 128-byte requirement) */
    hda.bdl = (hda_bdl_entry_t *)mem_alloc_pages(1);
    if (!hda.bdl) {
    ...
    hda.bdl[0].addr = (uint64_t)(uintptr_t)hda.audio_buf;
    ...
    hda_write32(sd + HDA_SD_BDLPL, (uint32_t)(uint64_t)(uintptr_t)hda.bdl);
    hda_write32(sd + HDA_SD_BDLPU, (uint32_t)((uint64_t)(uintptr_t)hda.bdl >> 32));
...
int hda_init(uint64_t bar0_phys, uint8_t bus, uint8_t dev, uint8_t func)
{
    ...
    hda.bar0 = (uintptr_t)bar0_phys;

    /* PCI bus master already enabled by pci_scan() */
