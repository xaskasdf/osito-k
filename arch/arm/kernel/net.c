/*
 * OsitoK AArch64 — Minimal Network Stack (port from x86-64)
 *
 * ARP responder + IPv4 + UDP + ICMP + TCP. Polling mode, static IP.
 * Uses VirtIO-net MMIO driver instead of i211.
 */
...
/* NIC interface: redirect i211_* to virtio_net_* */
#define i211_get_mac    virtio_net_get_mac
#define i211_send       virtio_net_send
#define i211_recv       virtio_net_recv
                    ...
                    uint16_t src_port)
{
    ...
    if (!arp_lookup(nexthop)) {
        ...
        uint64_t arp_start = timer_get_ticks();
        while (!arp_lookup(nexthop) && (timer_get_ticks() - arp_start) < 200) {
            net_poll();
    ...
    while (conn->state == TCP_SYN_SENT && (timer_get_ticks() - start) < 500) {
        net_poll();
        __asm__ volatile ("wfe");
    }
...
int net_dns_resolve(const char *hostname, uint8_t ip_out[4])
{
    ...
    uint64_t start = timer_get_ticks();
    while (!dns_got_reply && (timer_get_ticks() - start) < 300) {
        net_poll();
        __asm__ volatile ("wfe");
    }
