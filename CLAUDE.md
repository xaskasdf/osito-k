...
| W-CC    | In-browser C/C++ compilation (`cc` shell command): clang.wasm 30M + lld.wasm 19M + wasi-sysroot 9M (libc, libc++) hosted on R2; lazy-loaded Web Worker; bridge `EM_JS(js_cc_kick/done/drain)` + Asyncify polling = `tcc -run` style compile-link-execute pipeline. Ver [docs/wasm-cc.md](docs/wasm-cc.md) | PoC |
